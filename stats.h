...
public:
    ...
    // s to be 1 in those expressions.
	...
	* @param lr is (x,y) of the lower right corner of the rectangle */
    ...
    double getHueX(HSLAPixel* pixel);
    double getHueY(HSLAPixel* pixel);

    double getHueAvg(double hueX, double hueY);

    double getSumHueX(pair<int, int> xy);
    double getSumHueY(pair<int, int> xy);
    double getSumSat(pair<int, int> xy);
    double getSumLum(pair<int, int> xy);
